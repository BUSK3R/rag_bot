...
실제 공고문(25청크)과 KURE-v1로 세 방식을 비교했습니다(`scripts/eval_retrieval.py`,
2026-08-25). 정답 청크를 1등으로 올린 문항 수:
...
자연어 질문("지원 대상 나이 조건은?")에서는 밀집만으로 충분합니다. 무너지는 건
`제3조제1항`, `S-CoP`, `CCUS` 같은 **문맥 없는 정확 토큰 조회**에서고(3–4등까지 밀림),
BM25가 그걸 전부 1등으로 되돌립니다.

즉 하이브리드는 평균을 올리는 장치가 아니라 **보험**입니다: 조회형 질의에서의 붕괴를
막는 대신, 자연어 질문 한 문항이 1등 → 3등으로 밀립니다(RRF는 두 검색기가 서로 다른
정답 청크를 1등으로 꼽으면 양쪽 2–3등에 걸친 문서를 우선합니다). 기본 `top_k=4`에서는
3등도 근거에 포함되므로 실질 손실이 없습니다.
